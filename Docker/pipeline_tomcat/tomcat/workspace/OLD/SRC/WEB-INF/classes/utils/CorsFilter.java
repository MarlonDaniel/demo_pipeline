...
public class CorsFilter implements ContainerResponseFilter {
	...
	public ContainerResponse filter(ContainerRequest req, ContainerResponse crunchifyContainerResponse) {
        ...
        if ("OPTIONS".equals(req.getMethod())){//这里通过判断请求的方法，判断此次是否是预检请求，如果是，立即返回一个204状态吗，标示，允许跨域；预检后，正式请求，这个方法参数就是我们设置的post了
        	crunchifyResponseBuilder.status(200);
        	//req.sta; //HttpStatus.SC_NO_CONTENT = 204
        	crunchifyResponseBuilder.header("Access-Control-Allow-Methods", "POST, GET, DELETE, OPTIONS, DELETE")//当判定为预检请求后，设定允许请求的方法
        	.header("Access-Control-Allow-Headers", "Content-Type, x-requested-with, Token") //当判定为预检请求后，设定允许请求的头部类型
        	.header("Access-Control-Max-Age", "1");  // 预检有效保持时间                       
          }
